...
from PyQt5.QtWidgets import (
    ...
    QTreeWidget, QTreeWidgetItem, QMenu, QInputDialog, QAbstractItemView, QShortcut
)
...
class MainWindow(QMainWindow):
    ...
    def on_filtering_finished(self):
        ...
        filtered_file_path = self.data_filterer.get_output_path()  # Ensure this method exists in DataFilterer
        try:
    ...
    def setup_voice_tab(self):
        ...
        generate_second_pass_button = QPushButton("Generate Second Pass Commentary")
        generate_second_pass_button.clicked.connect(self.generate_second_pass)
        ...
        layout.addWidget(browse_button)
        layout.addWidget(generate_button)
        layout.addWidget(generate_second_pass_button)
        layout.addWidget(self.voice_output)

    def generate_second_pass(self):
        input_path = self.voice_input.text()
        if not input_path:
            input_path = self.voice_generator.get_output_dir() if hasattr(self, 'voice_generator') else None
        ...
        if not claude_api_key or not eleven_labs_api_key:
            QMessageBox.warning(self, "API Keys Missing", "Please enter both Claude and ElevenLabs API keys in the Settings tab.")
            return

        self.second_pass_commentator = SecondPassCommentator(input_path, claude_api_key, eleven_labs_api_key)
        self.second_pass_commentator.output_signal.connect(self.update_voice_output)
        self.second_pass_commentator.progress_signal.connect(self.update_progress_bar)
        self.second_pass_commentator.start()
